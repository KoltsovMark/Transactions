...
class CurrencyTest extends TestCase
{
    ...
    /**
     * @covers \CommissionTask\Service\Currency::getDefaultCurrenciesCodes
     */
    public function testGetSupportedCurrenciesCodes()
    {
       ...
       $this->assertEquals($supportedCurrencies, $this->currencyService->getDefaultCurrenciesCodes());
    }
     ...
     */
    public function testIsSupportedCurrencyCode(string $currencyCode, ?CurrencyModel $currency, bool $expectation)
    {
        $this->currencyRepositoryMock
            ->expects($this->once())
            ->method('getCurrencyByCodeOrNull')
            ->with(...[$currencyCode])
            ->willReturn($currency)
        ;

        $this->assertEquals($expectation, $this->currencyService->isSupportedCurrencyCode($currencyCode));
    }
    ...
    {
        return [
            'supported currency' => ['USD', (new CurrencyModel())->setCode('USD'), true],
            'unsupported currency' => ['UAH', null, false],
        ];
    }
